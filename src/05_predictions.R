...
for (spp in SPP) {
        ...
        if (type == "C") {
                ...
                north.sector <- north.sector[match(rownames(kgrid), rownames(north.sector)), ]
                north.sector[is.na(north.sector)] <- 0
                rownames(north.sector) <- rownames(kgrid)
                
                south.sector <- south.sector[match(rownames(kgrid), rownames(south.sector)), ]
                south.sector[is.na(south.sector)] <- 0
                rownames(south.sector) <- rownames(kgrid)
                
                sector.effect <- north.sector
                
                for (i in levels(chVeg$sector)) {
                        
                        sector.effect[, paste0("Cur_", i)] <- kgrid$wN * north.sector[, paste0("Cur_", i)] + (1-kgrid$wN) * south.sector[, paste0("Cur_", i)]
                        sector.effect[, paste0("Ref_", i)] <- kgrid$wN * north.sector[, paste0("Ref_", i)] + (1-kgrid$wN) * south.sector[, paste0("Ref_", i)]
                        
                }
                
                save(sector.effect,
                     file=file.path(paste0("data/processed/predictions/comb/", spp, "-comb_", Sys.Date(), ".RData")))
        ...
        if (type == "S") {
                ...
                save(sector.effect,
                     file=file.path(paste0("data/processed/predictions/south/", spp, "-south_", Sys.Date(), ".RData")))
        ...
        if (type == "N") {
                ...
                sector.effect <- north.sector
                save(sector.effect,
                     file=file.path(paste0("data/processed/predictions/north/", spp, "-north_", Sys.Date(), ".RData")))
                
        }
        
        rm(sector.effect, south.sector, north.sector)
