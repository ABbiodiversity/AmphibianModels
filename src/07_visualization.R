...
# Keywords: Occurrence, Seasonality, Coefficients, Climate Results, Relative abundance, Sector effects
# Notes: 
#
...
species.pred <- c("data/processed/predictions/comb/BCFR-comb_2022-01-20.RData", "data/processed/predictions/comb/WETO-comb_2022-01-20.RData",
                  "data/processed/predictions/comb/CATO-comb_2022-01-20.RData", "data/processed/predictions/comb/WOFR-comb_2022-01-20.RData")
species.names <- c("BCFR", "WETO", "CATO", "WOFR")
...
results.file <- list.files(path = "results/tables/SectorEffects/", full.names = TRUE)
names(results.file) <- c("North", "South")

for (name.id in names(results.file)) {
    
    #####################
    # Species Formating #
    #####################
    
    species.subset <- read.csv(results.file[name.id])
    
    # Filter for species predicted as TRUE
    species.subset <- species.subset[species.subset$Pred_Keep, ]
